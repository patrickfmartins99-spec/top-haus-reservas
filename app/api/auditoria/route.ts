...
export async function GET(request: Request) {
  ...
  if (actorIds.length) {
    const users = await context.authentication.getUsers(
      actorIds.map((uid) => ({ uid })),
    );
    for (const user of users.users) {
      actorNames.set(
        user.uid,
        user.displayName || user.email?.split('@')[0] || 'Colaborador',
      );
    }
  }
  ...
  const events = snapshot.docs.map((document) => {
    ...
    return {
      ...
      reason:
        typeof data.changes?.reasonLabel === 'string'
          ? data.changes.reasonLabel
          : null,
      note: typeof data.changes?.note === 'string' ? data.changes.note : null,
      tableChange:
        data.action === 'reservation_table_assigned'
          ? `${String(data.changes?.before?.tableLabel || 'não definida')} → ${String(data.changes?.after?.tableLabel || 'não definida')}`
          : null,
