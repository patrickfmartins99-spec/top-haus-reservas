...
export async function GET(request: Request) {
  ...
  const [reservationSnapshot, waitlistSnapshot, messageSnapshot, eventSnapshot] = await Promise.all([
    ...
    database.collection('whatsappQueue').where('status', '==', 'manual_pending').limit(100).get(),
    database.collection('staffNotifications').orderBy('createdAt', 'desc').limit(20).get(),
  ]);
  const items = [
    ...eventSnapshot.docs.map((document) => { const data = document.data(); return { id: `event_${document.id}`, title: String(data.title ?? 'Atualização no atendimento'), description: 'Abra para conferir os detalhes.', href: String(data.href ?? '/painel'), createdAt: timestamp(data.createdAt), priority: 0 }; }),
    ...(messageSnapshot.empty ? [] : [{ id: 'manual_messages', title: 'Mensagens aguardando envio manual', description: `${messageSnapshot.size} mensagens para conferir e enviar pelo WhatsApp.`, href: '/painel/mensagens', createdAt: new Date().toISOString(), priority: 3 }]),
