...
export default function MinhaReservaPage() {
  ...
  async function removeReservation() {
    ...
    setError('');
    try {
      const response = await fetch('/api/minha-reserva', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ code: reservation?.id, whatsapp }),
      });
      ...
      setDeleteOpen(false);
      setSuccess(
        'Reserva excluída. Os lugares foram liberados e a ação ficou registrada.',
      );
    } catch (error) {
      setError(
    ...
    if (updated) {
      ...
      setSuccess(
        updated.status === 'pending_approval'
          ? 'Alteração enviada e aguardando aprovação da equipe.'
          : 'Reserva alterada e confirmada.',
      );
    }
  }

  const supportUrl = reservation
    ? buildWhatsAppUrl(
        reservation.restaurantWhatsapp,
        `Olá! Preciso de atendimento sobre a reserva ${reservation.id}.`,
      )
    : '';
    ...
    <main className="min-h-screen bg-[#efede8] text-haus-ink">
      ...
      <section className="mx-auto grid max-w-6xl gap-8 px-5 py-10 sm:px-8 lg:grid-cols-[0.78fr_1.22fr] lg:py-16">
        <div className="max-w-md pt-3">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-haus-terracotta">
            Área do cliente
          </p>
          <h1 className="mt-3 text-4xl font-extrabold tracking-[-0.035em]">
            Acompanhe sua reserva.
          </h1>
          <p className="mt-4 leading-7 text-black/65">
            Consulte os dados reais, confirme a presença, altere ou cancele
            dentro do prazo.
          </p>
          <div className="mt-8 space-y-4 text-sm text-black/65">
            <p className="flex items-center gap-3">
              <span className="grid size-9 place-items-center rounded-full bg-white">
                <MessageCircle className="size-4 text-haus-terracotta" />
              </span>{' '}
              Use o mesmo WhatsApp informado na reserva.
            </p>
            <p className="flex items-center gap-3">
              <span className="grid size-9 place-items-center rounded-full bg-white">
                <Clock3 className="size-4 text-haus-terracotta" />
              </span>{' '}
              Fora do prazo, a equipe atende pelo WhatsApp.
            </p>
          </div>
        </div>
        <Card className="border-0 bg-[#fdfcf9] py-0 shadow-[0_24px_70px_rgba(0,0,0,0.12)] ring-black/5">
          <CardContent className="p-6 sm:p-8">
            {error ? (
              <p
                className="mb-5 rounded-xl bg-destructive/10 px-4 py-3 text-sm font-medium text-destructive"
                role="alert"
              >
                {error}
              </p>
            ...
            {success ? (
              <output className="mb-5 block rounded-xl bg-black px-4 py-3 text-sm font-semibold text-white">
                {success}
              </output>
            ) : null}
            {!reservation ? (
              <form onSubmit={findReservation} className="space-y-5">
                <div>
                  ...
                  <p className="mt-1 text-sm text-black/60">
                    Informe o código recebido ao reservar e o WhatsApp
                    cadastrado.
                  </p>
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Cole o código da reserva"
                    required
                  />
                </div>
                <div className="space-y-2">
                ...
                <Button
                  type="submit"
                  disabled={loading}
                  className="h-12 w-full bg-haus-terracotta text-base font-bold text-white hover:bg-haus-terracotta/90"
                >
                  {loading ? (
              ...
              <div className="space-y-6">
                <div className="flex flex-wrap items-start justify-between gap-4 border-b border-black/8 pb-5">
                  <div>
                    ...
                    <h2 className="mt-2 text-2xl font-bold">
                      {reservation.service === 'almoco' ? 'Almoço' : 'Rodízio'}{' '}
                      no Top Haus
                    </h2>
                    <p className="mt-1 text-sm font-medium text-black/65">
                      Em nome de {reservation.customerName}
                    </p>
                  </div>
                  <span className="rounded-full bg-[#eadcd2] px-3 py-1.5 text-xs font-bold text-[#653b25]">
                    {statusLabels[reservation.status] ?? reservation.status}
                  </span>
                </div>
                <div className="grid gap-3 sm:grid-cols-3">
                  <div className="rounded-2xl bg-white p-4 ring-1 ring-black/6">
                    <CalendarDays className="size-5 text-haus-terracotta" />
                  ...
                  <div className="rounded-2xl bg-white p-4 ring-1 ring-black/6">
                    <Clock3 className="size-5 text-haus-terracotta" />
                    <p className="mt-3 text-xs font-medium text-black/60">
                      Chegada
                    </p>
                    <p className="mt-1 font-bold">{reservation.arrivalTime}</p>
                  </div>
                  <div className="rounded-2xl bg-white p-4 ring-1 ring-black/6">
                    <Users className="size-5 text-haus-terracotta" />
                    <p className="mt-3 text-xs font-medium text-black/60">
                      Pessoas
                    </p>
                    <p className="mt-1 font-bold">{reservation.partySize}</p>
                  </div>
                </div>
                <div className="rounded-2xl border border-haus-gold/40 bg-[#f4e7d7] p-4 text-sm leading-6 text-black/75">
                  A mesa será mantida por {reservation.lateToleranceMinutes}{' '}
                  minutos após o horário de chegada. Crianças e bebês devem
                  estar incluídos na quantidade.
                </div>
                {reservation.status === 'confirmed' ? (
                  <Button
                    ...
                    className="h-12 w-full bg-black text-base font-bold text-white hover:bg-black/85"
                  >
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  {reservation.canModify ? (
                    <Button
                      ...
                      variant="outline"
                      className="h-11"
                      onClick={() => setDraft({ ...reservation })}
                    >
                      <Pencil /> Alterar reserva
                    </Button>
                  ) : supportUrl ? (
                    <a
                      href={supportUrl}
                      ...
                      className={buttonVariants({
                        variant: 'outline',
                        className:
                          'h-11 border-haus-terracotta/30 text-haus-terracotta',
                      })}
                    >
                ...
                {(reservation.canModify ||
                  (reservation.status === 'cancelled' &&
                    new Date(reservation.modifyDeadline).getTime() >=
                      Date.now())) && (
                  <Button
                    type="button"
                    variant="outline"
                  ...
                  >
                    <Trash2 /> Excluir reserva do sistema
                  </Button>
                )}
                {reservation.canModify &&
          ...
          {draft ? (
            <form onSubmit={saveReservation}>
              <DialogHeader>
                ...
                <DialogDescription>
                  As alterações ficam registradas e podem exigir nova aprovação.
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-5 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  <Label htmlFor="client-party">Pessoas</Label>
                  <Input
                    id="client-party"
                    type="number"
                    min={1}
                ...
                <div className="space-y-2">
                  <Label htmlFor="client-date">Data</Label>
                  <Input
                    id="client-date"
                    type="date"
                ...
                <div className="space-y-2">
                  <Label htmlFor="client-service">Serviço</Label>
                  <NativeSelect
                    id="client-service"
                    value={draft.service}
                    onChange={(event) => {
                ...
                <div className="space-y-2">
                  <Label htmlFor="client-time">Horário</Label>
                  <NativeSelect
                    id="client-time"
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Excluir esta reserva?</DialogTitle>
            <DialogDescription>
              Ela sairá das listas e os lugares serão liberados. O histórico de
              auditoria será preservado. Esta ação não pode ser desfeita pelo
              site.
            </DialogDescription>
          </DialogHeader>
          {error && (
      ...
      <Dialog open={cancelOpen} onOpenChange={setCancelOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Os lugares serão liberados imediatamente. Esta ação ficará
              registrada.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
