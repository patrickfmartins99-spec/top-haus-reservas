...
import { CalendarDays, LoaderCircle, MessageCircle, Pencil, Plus, Save, Search, Trash2 } from 'lucide-react';
...
export default function ReservationsPage() {
  ...
  const [dateFilter, setDateFilter] = useState(() => searchParams.get('busca') || searchParams.get('criada') ? '' : new Intl.DateTimeFormat('en-CA', {timeZone:'America/Sao_Paulo'}).format(new Date()));
  ...
  const filteredReservations = useMemo(() => {
    ...
    }).sort((a,b)=>a.serviceDate.localeCompare(b.serviceDate)||a.arrivalTime.localeCompare(b.arrivalTime));
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-6 p-5 sm:p-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-haus-terracotta">Atendimento</p><h1 className="mt-2 text-3xl font-extrabold tracking-[-0.03em]">Reservas</h1><p className="mt-1 text-xs text-black/75">Consulte e acompanhe todas as reservas registradas no Firebase.</p></div>
      ...
      <Card className="bg-white ring-black/7">
        <CardHeader className="gap-4 border-b border-black/7 lg:flex-row lg:items-center lg:justify-between">
          <div><CardTitle className="text-xl font-bold">{dateFilter ? 'Reservas do dia' : 'Todas as reservas'}</CardTitle><p className="mt-1 text-sm text-black/65">Use os filtros ou escolha outra data para localizar um atendimento.</p></div>
        ...
        <CardContent className="min-w-0"><div className="mb-4 flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={()=>setDateFilter(new Intl.DateTimeFormat('en-CA',{timeZone:'America/Sao_Paulo'}).format(new Date()))}>Hoje</Button><Button variant="outline" size="sm" onClick={()=>setDateFilter('')}>Todas as datas</Button></div>
          ...
          {!loading && filteredReservations.length > 0 ? <ReservationCards items={filteredReservations} actions={item=>{const reservation=filteredReservations.find(r=>r.id===item.id)!;return <><Button variant="outline" size="sm" onClick={()=>startEditing(reservation)}><Pencil/> Editar</Button><Button variant="outline" size="sm" className="text-red-700" onClick={()=>{setError('');setDeleting(reservation);}}><Trash2/> Excluir</Button><Link href="/painel/mensagens" className={buttonVariants({variant:'outline',size:'sm'})}><MessageCircle/> Mensagens</Link></>;}}/> : null}
        </CardContent>
      </Card>
