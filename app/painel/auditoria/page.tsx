...
const actionLabels: Record<string, string> = {
  ...
  reservation_arrived: 'Confirmou a chegada',
...
function eventDetail(event: AuditEvent) {
  ...
  const statusChange =
    event.fromStatus && event.toStatus && event.fromStatus !== event.toStatus
      ...
      : '';
  return `${code ?? 'Registro'}${statusChange}${event.tableChange ? ` · Mesa: ${event.tableChange}` : ''}${event.reason ? ` · Motivo: ${event.reason}` : ''}${event.note ? ` · ${event.note}` : ''}`;
}
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(firebase.auth, async (user) => {
      ...
      try {
        ...
        if (!response.ok)
          throw new Error(
            data.error ?? 'Não foi possível carregar a auditoria.',
          );
    ...
    <div className="mx-auto max-w-6xl space-y-6 p-5 sm:p-8">
      <div>
        ...
        <p className="mt-1 text-sm text-black/65">
          Histórico real de alterações realizadas por clientes, colaboradores e
          automações.
        </p>
      </div>
      {error ? (
      ...
      <Card className="bg-white ring-black/7">
        <CardHeader className="flex-row items-center justify-between border-b border-black/7">
          <div>
            <CardTitle className="flex items-center gap-2">
              <History className="size-5 text-haus-terracotta" /> Atividades
              recentes
            </CardTitle>
            <p className="mt-1 text-sm text-black/65">
              Registros salvos automaticamente no Firebase.
            </p>
          </div>
          <ShieldCheck className="size-6 text-black/60" />
        </CardHeader>
        <CardContent className="overflow-x-auto">
          ...
          {!loading && events.length === 0 ? (
            <p className="py-12 text-center text-sm text-black/65">
              Nenhuma atividade registrada. As próximas ações aparecerão aqui.
            </p>
          ) : null}
          {!loading && events.length > 0 ? (
            <Table>
              ...
              <TableBody>
                {events.map((event) => (
                  <TableRow key={event.id}>
                    <TableCell className="whitespace-nowrap font-mono text-xs font-semibold text-black/70">
