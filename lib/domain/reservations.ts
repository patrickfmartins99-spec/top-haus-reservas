...
export function bookingDeadline(
  input: Pick<
    ...
    'service' | 'serviceDate' | 'arrivalTime'
  >,
  lunchAdvanceHours = 24,
) {
  return input.service === 'rodizio'
    ? new Date(`${input.serviceDate}T${DINNER_BOOKING_CUTOFF}:00-03:00`)
    : new Date(
        reservationInstant(input).getTime() - lunchAdvanceHours * 3_600_000,
      );
}
...
export function minimumBookingDate(
  service: ServiceType,
  lunchAdvanceHours = 24,
  now = Date.now(),
) {
  if (service === 'almoco')
    return brazilDate(new Date(now + lunchAdvanceHours * 3_600_000));
  const today = brazilDate(new Date(now));
  return now <= new Date(`${today}T18:00:00-03:00`).getTime()
    ? today
    : brazilDate(new Date(now + 86_400_000));
}
...
export function isReservationInput(
  value: unknown,
): value is CreateReservationInput {
  ...
  return (
    ...
    new Date(`${input.serviceDate}T12:00:00Z`).toISOString().slice(0, 10) ===
      input.serviceDate &&
    typeof input.arrivalTime === 'string' &&
    /^(?:[01]\d|2[0-3]):[0-5]\d$/.test(input.arrivalTime) &&
...
export function reservationInstant(
  input: Pick<CreateReservationInput, 'serviceDate' | 'arrivalTime'>,
) {
  return new Date(`${input.serviceDate}T${input.arrivalTime}:00-03:00`);
}

export function isAtLeastTwentyFourHoursAhead(
  input: Pick<CreateReservationInput, 'serviceDate' | 'arrivalTime'>,
) {
  return (
    reservationInstant(input).getTime() - Date.now() >=
