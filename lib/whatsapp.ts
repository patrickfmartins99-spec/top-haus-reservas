...
export const messageTitles: Record<string, string> = {
  ...
  reservation_pending_approval: 'Solicitação recebida',
  reservation_approved: 'Reserva aprovada',
  reservation_updated: 'Reserva alterada',
  reservation_cancelled: 'Reserva cancelada',
  reservation_presence_confirmed: 'Presença confirmada',
  reservation_no_show: 'Reserva encerrada por ausência',
  reservation_seated: 'Boas-vindas ao Top Haus',
  reservation_completed: 'Obrigado pela visita',
  waitlist_created: 'Entrada na fila',
  waitlist_updated: 'Dados da fila atualizados',
  waitlist_called: 'Sua mesa está disponível',
  waitlist_seated: 'Boas-vindas ao Top Haus',
  waitlist_removed: 'Saída da fila',
};
...
export function customerMessage(event: string, data: MessageData) {
  ...
  if (event === 'reservation_pending_approval')
  ...
  if (event === 'reservation_confirmed' || event === 'reservation_approved')
  ...
  if (event === 'reservation_updated') {
    ...
    const format = (key: string, value: unknown) =>
      key === 'serviceDate'
        ? textValue(value).split('-').reverse().join('/')
        : key === 'service'
          ? value === 'almoco'
            ? 'Almoço'
            : 'Rodízio'
    ...
    const changes = Object.entries(labels)
      .filter(
        ([key]) =>
          data.previous &&
          textValue(data.previous[key]) !==
            textValue(data[key as keyof MessageData]),
      )
      .map(
        ([key, label]) =>
          `• ${label}: ${format(key, data.previous![key])} → *${format(key, data[key as keyof MessageData])}*`,
      )
      .join('\n');
  ...
  if (event === 'reservation_presence_confirmed')
  ...
  if (event === 'reservation_completed')
          ...
          : reservation.status === 'completed'
            ...
            : reservation.status === 'seated'
              ...
              : reservation.status === 'presence_confirmed'
                ...
                : 'reservation_confirmed';
  return customerMessage(event, {
    ...reservation,
    reservationCode: reservation.reservationCode || reservation.id,
  });
}
